...
\begin{document}
	...
	\newglossaryentry{batchsize}{
		name=Batch-Size,
		description={Die Anzahl an Trainingsbeispielen, die in einem Schritt (Iteration) gleichzeitig durch das neuronale Netz propagiert und zur Gradientenberechnung genutzt werden. 
			Eine kleinere Batch Size führt zu häufigeren Gewichtsanpassungen, während eine größere Batch Size stabilere Gradienten liefert, aber mehr Speicher benötigt}
	}
	...
	\newglossaryentry{regularisierungstechnik}{
		name=Regularisierungstechnik,
		description={Methoden zur Vermeidung von Overfitting in maschinellen Lernmodellen. Durch Regularisierung wird die Modellkomplexität gezielt eingeschränkt oder die Lernparameter stabilisiert, beispielsweise durch Gewichtsbeschränkungen (L1- oder L2-Regularisierung) oder Dropout. Ziel ist die Verbesserung der Generalisierungsfähigkeit des Modells auf unbekannte Daten.}
	}
	...
	Zwar bietet Keycloak regelbasierte Sicherheitsmaßnahmen an- diese beruhen jedoch größtenteils auf vordefinierten Bedingungen. Dadurch können sie lediglich bekannte Muster erkennen und sind wenig flexibel gegenüber neuen oder komplexeren Angriffstechniken \cite[S. 3]{waltl2017rulebased}. 
	Neue, bislang unbekannte Angriffsarten (sogenannte \gls{zerodayangriff}e) entziehen sich dieser Logik, da keine entsprechenden Regeln vorliegen. Angreifende nutzen gezielt sogenannte \textit{Adversarial Attacks},
	...
	\\[0.5em]
	Ein weiteres Beispiel ist der integrierte Brute-Force-Schutz von Keycloak, der jedoch nachweislich umgangen werden kann – etwa durch sogenannte \textit{Timing-Angriffe} \footnote{National Vulnerability Database (NVD), „CVE-2024-4629 Vulnerability Details“, \url{https://nvd.nist.gov/vuln/detail/CVE-2024-4629}, letzter Zugriff: 30. Juli 2025.}. Dabei analysiert ein Angreifer die Antwortzeit des Systems, um Rückschlüsse auf den Authentifizierungsprozess zu ziehen. Wird beispielsweise ein korrekter Benutzername, aber ein (fast) korrektes Passwort eingegeben, kann die Antwortzeit minimal verlängert sein – was auf die sequentielle Überprüfung der Passwortzeichen hindeutet. Solche zeitlichen Unterschiede im Millisekundenbereich lassen sich mit ausreichend vielen Versuchen systematisch auswerten.
	\\[0.5em]
	...
	Für jede Teststatistik werden üblicherweise der \textbf{F-Wert} und der zugehörige \textbf{p-Wert} angegeben. Ein kleiner p-Wert (z.B. $p < 0.05$) weist darauf hin, dass mindestens eine Gruppe signifikant von den anderen abweicht. Die MANOVA ist besonders nützlich, wenn abhängige Variablen korreliert sind, da sie die gemeinsame Varianz berücksichtigt und das Risiko von Typ-I-Fehlern \footnote{Typ-1-Fehler: Man nimmt an, dass die Nullhypothese korrekt ist, obwohl sie eigentlich widerlegt wurde} reduziert \cite[S.3]{Warne2014}.
	...
	\\[0.5em]
	Da die Generierung synthetischer Logs Herausforderungen mit sich bringt, wird ebenfalls in Betracht gezogen, echte Daten zu verwenden. Diese können z. B. durch automatisierte Tests (auf die im weiteren Verlauf der Arbeit eingegangen wird) generiert werden. Auch firmeninterne Daten wurden erwogen. Schlussendlich werden generierte Logs verwendet- da diese nicht direkt dem realen Benutzerverhalten entsprechen hat man schon mal diesbezüglich eine große Herausforderung in der Methodischen Umsetzung der Angriffsszenarien und generell in der Log-Generierung.
	\\[0.5em]
	Da solche Daten häufig begrenzt verfügbar sind, stehen meist nur wenige Trainings- und Testdaten zur Verfügung. LSTMs gelten umgangssprachlich als „datenhungrig“, da sie für die Analyse hochkomplexer zeitbasierter Daten entwickelt wurden und typischerweise eine große Menge an Logs benötigen. Ein zu kleiner Datensatz könnte daher zu einer Unterforderung des Modells führen, was die Ergebnisse erheblich beeinflusst. Da auch in dieser Arbeit ein kleiner Datensatz verwendet wird, ist dies zu berücksichtigen.
	\\[0.5em]
	...
	\subsection{Keras}
	\gls{keras}\footnote{Keras: The Python Deep Learning API. Offizielle Dokumentation, verfügbar unter: \url{https://keras.io/}, letzter Zugriff: 30. Juli 2025.} ist eine benutzerfreundliche, modulare und erweiterbare Open-Source-Bibliothek für die Entwicklung von Deep-Learning-Modellen in Python. Sie bietet eine einfache und intuitive API, die das schnelle Prototyping, die Implementierung und das Training neuronaler Netze ermöglicht. Keras unterstützt verschiedene neuronale Netzwerkarchitekturen, darunter Convolutional Neural Networks (CNNs), Recurrent Neural Networks (\gls{rnn}s) und Autoencoder.
	...
	\\[0.5em]
	Keycloak basierte in früherer Zeit \textbf{\gls{wildfly}}-Anwendungsserver\footnote{WildFly Project, \url{https://www.wildfly.org/}, letzter Zugriff: 30. Juli 2025.}
	...
	JBoss Logging dient in Keycloak als zentrale Logging-API. Es abstrahiert verschiedene Logging-Backends (wie \texttt{\gls{log4j}} und \texttt{\gls{slf4j}}) und ermöglicht die strukturierte Ausgabe sicherheitsrelevanter Informationen.
	...
	\\[0.5em]
	Zur Orientierung wie die Logs generiert werden können und mit welchen Features, wurden die Logs durch JBoss Logging zunächst ausgegeben über ein Terminal. In späterer Entwicklung wurde dann entschieden, die Keycloak-API zu verwenden, auf welche in den späteren Kapiteln noch eingegangen wird.
	...
	\end{itemize}
	Die \gls{adminlogs} enthalten alle Logs bzgl. den Operationen des Admins und die User Logs nur die Logs von Benutzern, die nicht Admin sein.
	...
	\\[0.5em]
	Des weiteren unterteilt Keycloak noch sogenannte \gls{loglevel}. Diese zeigen, ob eine bestimmte Operation erfolgreich war oder nicht oder geben andere Informationen bzgl. einer Operation.
	...
	Bspw. ist LOGIN ein Ereignis-Typ bei dem der Benutzer sich anmeldet. Dies wird in Keycloak auch regelmäßig dokumentiert. Andere Ereignistypen sind bspw. \gls{refreshtoken}, bei dem der Benutzer einen neuen Token bekommt. 
	...
	\end{figure}
	Dieser Trainingsfehler, bzw. Rekonstruktionsfehler, also die Daten, welche nicht klar klassifiziert werden konnten, werden danach jeweils mit einem der Anomalieerkennungs-Algorithmen trainiert. Dies ist auch eine Vorangehensweise, welche bspw. in der Studie von Malhotra bekannt ist \cite{malhotra2016lstm}.
	\\[0.5em]
	Dadurch kann jedes Modell separat trainiert, getestet und evaluiert werden, was eine modulare Struktur gewährleistet, auch wenn die gesamte Umsetzung in einem \gls{jupyternotebook} erfolgt.
